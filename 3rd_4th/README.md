...
## Elasticsearch 환경설정 - 그 외 시스템 설정 (Keynote P22~23)
...
[ec2-user@ip-xxx-xxx-xxx-xxx ~]$ curl -H 'Content-Type: application/x-ndjson' -XPOST 'localhost:9200/shakespeare/doc/_bulk?pretty' --data-binary @shakespeare_6.0.json

[ec2-user@ip-xxx-xxx-xxx-xxx ~]$ curl -H 'Content-Type: application/x-ndjson' -XPOST 'localhost:9200/_bulk?pretty' --data-binary @logs.jsonl
```
